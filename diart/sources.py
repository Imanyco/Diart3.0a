...
class StreamAudioSource(AudioSource):
    ...
    def put_frame(self, frame):
        """Put the current frame to frame queue"""
    ...
    def read(self):
        ...
        while not self.is_closed:
            ...
            # append current buffer to the waveform data
            stream_data += float_data

            # make waveform
            stream_len = len(stream_data)
            while stream_len > self.block_size:
                # get current block size of wave form
                waveform = [stream_data[:self.block_size]]

                # update stream data
                stream_data = stream_data[self.block_size:]
                stream_len = len(stream_data)
                ...
                try:
                    if self.is_closed:
                        break
                    self.stream.on_next(np.array(waveform))
                except BaseException as e:
                    self.stream.on_error(e)
                    break
